    ...
    <header className="relative mx-auto max-w-6xl px-5 pb-10 pt-12 md:pt-20 md:pb-14">
      <div className="text-center">
        ...
        <div className="mt-7 flex flex-wrap items-center justify-center gap-2.5 text-xs">
          <Link href="/quantum" className="rounded-full border border-quantum-500/40 bg-quantum-900/40 px-4 py-1.5 text-quantum-200 transition hover:border-quantum-300/60">
            ⚛︎ 量子电路怎么跑
          </Link>
          <Link href="/about" className="rounded-full border border-gold-500/40 bg-ink-900/40 px-4 py-1.5 text-gold-200 transition hover:border-gold-300/60">
            ☯ 项目缘起
          </Link>
          <Link href="/disclaimer" className="rounded-full border border-cinnabar-500/40 bg-cinnabar-700/15 px-4 py-1.5 text-cinnabar-300 transition hover:border-cinnabar-400/60">
            ⚠ 免责声明
          </Link>
        </div>
      </div>
...
function DisclaimerBanner() {
  return (
    <div className="mx-auto mb-6 max-w-3xl">
      <div className="flex items-start gap-3 rounded-md border border-cinnabar-500/30 bg-cinnabar-700/10 px-4 py-3 text-[12px] leading-relaxed text-ink-200">
        <span className="mt-0.5 shrink-0 text-cinnabar-400">⚠</span>
        <p>
...
function FeatureGrid() {
  const features = [
    ...
    },
    {
      icon: "🤖",
      title: "AI 结构化解卦",
      desc: "DeepSeek V4 + 强结构化 prompt：辨象 → 取辞 → 观结构 → 看动向 → 参互错综 → 综合，每步引用原文。",
      tag: "DEEPSEEK",
      color: "gold",
    },
    {
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 pb-20">
      <div className="text-center">
        <p className="font-display text-[10px] tracking-[0.4em] text-gold-400">FEATURES</p>
        <h2 className="mt-2 font-display text-2xl text-gold-200 md:text-3xl">
          为什么这个项目和别的"占卜小工具"不一样
        </h2>
      </div>
        ...
        {features.map((f) => (
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              <span className="grid h-10 w-10 place-items-center rounded-md border border-gold-500/30 bg-ink-900/50 text-xl">
                {f.icon}
              </span>
              <span className="font-mono text-[10px] tracking-[0.3em] text-ink-400">{f.tag}</span>
            </div>
